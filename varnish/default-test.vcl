...
sub vcl_recv {
    ...
    #SED# If using https and no HAProxy, which is the case with AWS ELB, redirect non-https and non-www URLs to https://www.example.com
    #SEDif ( (req.http.host ~ "^(?i)www.example.com" && req.http.X-Forwarded-Proto !~ "(?i)https") || (req.http.host ~ "^(?i)example.com") ) {
